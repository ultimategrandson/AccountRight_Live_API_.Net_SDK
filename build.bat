...
REM .\packages\OpenCover.4.5.3207\OpenCover.Console.exe -register:user -filter:"+[MYOB.*]*" -skipautoprops -output:".\Artefacts\Coverage\output.xml" -target:".\packages\NUnit.Runners.2.6.3\tools\nunit-console.exe" -targetargs:".\Artefacts\SDK\Test\Release\SDK.Tests.dll /noshadow "
REM .\packages\ReportGenerator.1.9.1.0\ReportGenerator.exe -reports:.\Artefacts\Coverage\output.xml -targetdir:.\Artefacts\Coverage

REM .\.nuget\Nuget.exe pack MYOB.API.SDK\SDK.Package\Package.nuspec
